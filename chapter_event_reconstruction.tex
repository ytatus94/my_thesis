...
The di-tau invariant mass $m_{\tau\tau}$ is used in this analysis and addressed in Sect.~\ref{}.
...
The event selection criteria for signal region are summarized in Table~\ref{tab:event_signal_region}.
%
\begin{itemize}
    ...
    \item $\mathbf{N}_{\mathbf{b}\textbf{-jet}}$: Requiring $N_{b\textrm{-jet}} = 0$, the \ttbar backgrounds can be reduced significantly.
    ...
    \item $\mathbf{\met/H_{T}^{lep}}$: The scalar sum of the lepton transverse momentum is defined  in Eq.~\ref{eq:event_HT}.
    The $\met/H_{\textrm{T}}^{\textrm{lep}}$ variable is larger in the compressed signals but is smaller for the background such as \ttbar and diboson.
    %
    \begin{equation}
